...
import classOfYunLe
import math
import ctrl_algorithm.steering_ctrl as steering_ctrl
import classOfDataRecord
from socket import *
import struct
import threading

from ctrl_algorithm.steering_ctrl import new_pcontrol
...
if __name__ == "__main__":
    ...
    while i <= length-2:  # 当跟踪的最后一个点时候，结束跟踪，小车制动。因为每步只跟踪一个点，所以i的范围可以从0~length-1,设为len-2保险
        ...
        if is_arrive(vehicle_x_y, path_x_y, i) == False:
            ...
            D = 0.05
            current_speed = v * 3.6
            cmd_v = new_pcontrol(target_speed,current_speed,P,I,D )
            # 直接给定速度
            # cmd_v = 4/3.6
