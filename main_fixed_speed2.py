...
path_x_y = []
path_v = []
with open('pathRecord/行驶时记录数据pcontrol(2)2023-06-05 20:32:28.csv', 'r') as fileopen:
...
if __name__ == "__main__":
    ...
    while i <= length-2:  # 当跟踪的最后一个点时候，结束跟踪，小车制动。因为每步只跟踪一个点，所以i的范围可以从0~length-1,设为len-2保险
        ...
        '''
        小车测试点：
        1、测试组合导航输出的速度是否为小车行驶的真实速度
           1）这个可以通过小车在平地上和坡地上行驶同样的速度，然后看组合导航输出的速度是多少
        2、知道小车的速度对应的车轮转速
        3、组合导航差不多以0.14秒一次记录数据，平均一秒记录七次
        3、封装一个函数，实时的控制的小车的速度
           1）通过cmd_v这个变量进行控制
           2）这样的话，首先要获取每块农田的dem数据，然后根据不同的农机，需要输入不同的参数，
        '''
        ...
        #     # 速度控制pid实现
        #     current_speed = v * 3.6
        #     cmd_v = PControl(target_speed, current_speed)  # target_speed是km/h,v是m/s
        #     cmd_v = target_speed+cmd_v
        #     # 直接给定速度
        #     # cmd_v = 4/3.6
        #     cmd_v = 1.8
        #
     ...
     #   print(cmd_v)
      ...
      #  car.sendMsg(1, 1, cmd_steering, cmd_v, 0)
        car.sendMsg(1, 1, 0.0, path_v[i], 0)
        time.sleep(1.5)
